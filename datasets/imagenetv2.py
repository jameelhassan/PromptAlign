...
class ImageNetV2(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        self.dataset_dir = os.path.join(root, self.dataset_dir)
        image_dir = "imagenetv2-matched-frequency-format-val"
        self.image_dir = os.path.join("/nfs-stor/users/hanan.ghani/TPT/imagenetv2", image_dir)

        #text_file = os.path.join(self.dataset_dir, "classnames.txt")
        text_file = os.path.join('/nfs-stor/users/hanan.ghani/TPT/imagenetv2', "classnames.txt")
        classnames = ImageNet.read_classnames(text_file)
